...
import numpy as np
import copy
from ccpy.eomcc.cvs_ipeom3_intermediates import get_cvs_ipeom3_intermediates
...
def HR(dR, R, T, H, flag_RHF, system):
    ...
    # Get intermediates
    X = get_cvs_ipeom3_intermediates(H, R, core)
    # update R1
...
def build_HR_2A(R, Ra, Raa, Rab, Raaa, Raab, T, H, X, core):
    """Calculate the projection <ijb|[ (H_N e^(T1+T2))_C*(R1h+R2h1p) ]_C|0>, where i or j belong to core index."""
...
def build_HR_3A(R, Raa, Raaa, Raab, T, X, H, core):
    """Calculate the projection <ijkbc|[ (H_N e^(T1+T2))_C*(R1h+R2h1p+R3h2p) ]_C|0>, where i or j or k belong to core index."""
