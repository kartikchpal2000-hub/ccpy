...
def get_active_cvs_3h2p_pspace(system, num_active=1, target_irrep=None):
    from ccpy.utilities.active_space import active_hole, active_particle
    ...
    def active_core_valence_hole(hole_list, nact_o, nocc, ncore=system.num_act_core):
        core_hole = any(0 <= x < ncore for x in hole_list)
        valence_hole = any(nocc-nact_o <= x < nocc for x in hole_list)
        return 1 if (core_hole and valence_hole) else 0


    def active_core_valence_hole_2(hole_list_alpha, hole_list_beta, nact_o, nocc_a, nocc_b, ncore=system.num_act_core):
        core_hole = any(0 <= x < ncore for x in hole_list_alpha)
        valence_hole = any(nocc_a-nact_o <= x < nocc_a for x in hole_list_alpha) or  any(nocc_b-nact_o<= x < nocc_b for x in hole_list_beta)
        return 1 if (core_hole and valence_hole) else 0
    ...
    for i in range(system.noccupied_alpha):
        for j in range(i + 1, system.noccupied_alpha):
            for k in range(j + 1, system.noccupied_alpha):
                #if count_active_occ_alpha([i, j, k]) < num_active: continue
                if active_core_valence_hole([i, j, k], nact_o, system.noccupied_alpha) < num_active: continue
                for b in range(system.nunoccupied_alpha):
                    for c in range(b + 1, system.nunoccupied_alpha):
                        if not checksym_aaa(i, j, k, b, c): continue
                        excitations["aaa"].append([b + 1, c + 1, i + 1, j + 1, k + 1])
    ...
    for i in range(system.noccupied_alpha):
        for j in range(i + 1, system.noccupied_alpha):
            for k in range(system.noccupied_beta):
                #if (count_active_occ_alpha([i, j]) + count_active_occ_beta([k])) < num_active: continue
                if active_core_valence_hole_2([i, j], [k], nact_o, system.noccupied_alpha, system.noccupied_beta) < num_active: continue
                for b in range(system.nunoccupied_alpha):
                    for c in range(system.nunoccupied_beta):
                        if not checksym_aab(i, j, k, b, c): continue
                        excitations["aab"].append([b + 1, c + 1, i + 1, j + 1, k + 1])
